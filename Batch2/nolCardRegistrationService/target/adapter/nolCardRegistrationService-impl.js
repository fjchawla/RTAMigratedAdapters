...
function validateRegistration(language,userId, params) {

	var request = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xs="http://www.rta.ae/ActiveMatrix/ESB/CustomHeader/XMLSchema/" xmlns:xs1="http://www.rta.ae/ActiveMatrix/ESB/NolCardRegistrationService/XMLSchema">'
		+getHeader(language,userId)
		+'<soapenv:Body>'
			+'<xs1:cardRegistrationValidationRequest>'
				+jsonToXml(params,'', null)	
		    +'</xs1:cardRegistrationValidationRequest>'
		+'</soapenv:Body>'
	+'</soapenv:Envelope>';
	
	var result = invokeWebService(request,"validateRegistration");
	
	return result;

}

function submitRegistration(language,userId, params) {

	var request = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xs="http://www.rta.ae/ActiveMatrix/ESB/CustomHeader/XMLSchema/" xmlns:xs1="http://www.rta.ae/ActiveMatrix/ESB/NolCardRegistrationService/XMLSchema">'
		+getHeader(language,userId)
		+'<soapenv:Body>'
			+'<xs1:cardRegistrationSubmitRequest>'
				+jsonToXml(params,'', null)	
		    +'</xs1:cardRegistrationSubmitRequest>'
		+'</soapenv:Body>'
	+'</soapenv:Envelope>';
	
	var result = invokeWebService(request,"submitRegistration");
//	return request={
//	request:request,
//	result:result
//	} ;
