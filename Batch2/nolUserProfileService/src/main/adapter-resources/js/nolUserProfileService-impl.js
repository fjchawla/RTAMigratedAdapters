...
function syncMobileUserProfile(language,userId,params) {
	
	var request = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xs="http://www.rta.ae/ActiveMatrix/ESB/CustomHeader/XMLSchema/" xmlns:_0="http://www.rta.ae/ActiveMatrix/ESB/MobileUserProfileService/3_0">'
		+getHeader(language,userId)
		+'<soapenv:Body>'
			+'<_0:mobileUserProfileSyncRequest>'
				+jsonToXml(params,'', null)	
		    +'</_0:mobileUserProfileSyncRequest>'
		+'</soapenv:Body>'
	+'</soapenv:Envelope>';
	
	var result = invokeWebService(request,"mobileUserProfileSyncRequest");
...
function invokeWebService(request,soapAction){
	...
	try {
	    ...
	    var input = {
	        ...
	        path : "mobuserprofileexternalservice",
